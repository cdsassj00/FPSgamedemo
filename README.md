...
- **AI 생성 아날로그 호러 크리처 5종** — Higgsfield 컨셉 아트 → Meshy image-to-3D로
  메시·텍스처·리깅·전용 걸음 애니메이션까지 자동 생성:
  | 크리처 | 특징 | 걸음새 |
  |---|---|---|
  | 하운드 | 개 두개골 머리, 빠른 추격 | 비틀거림 |
  | 크롤러 | 웅크린 점토 피부, 가장 빠름 | 살금살금 |
  | 그리너 | 찢어진 정장의 웃는 남자 | 다리를 끄는 걸음 |
  | 위도우 | 창백한 인형 여자, 강함 | 비틀거림 |
  | 스토커 | **4.3m 거인**, 동시에 1체만 출현하는 보스 | 느린 비틀거림 |

  땅을 뚫고 기어 나오고, 얼굴엔 붉은 빛이 감돌며, 비명을 지르며 돌진합니다(스토커 제외 — 그놈은 뛸 필요가 없습니다). 웨이브가 오를수록 강한 개체의 비율이 늘어납니다
